...
```{r Anàlisi de les dades - Creació variable Month}
...
#fifaNet <- fifaNet %>% mutate(Age_Int = case_when(Age<=20  ~ "Junior",
#                                     Age>=21 & Age<=27   ~ "Middle",
#                                     Age>=28   ~ "Senior",))
```


```{r Anàlisi de les dades - Selecció dels grups de dades}
...
```{r Anàlisi de les dades - Humidity + Presure }
...
```{r Anàlisi de les dades 2 - Humidity + WindSpeed}
...
```{r Anàlisi de les dades 3 - Humidity + Evaporation }
...
```{r Anàlisi de les dades 4 - Humidity + Sunshine }
...
```{r Anàlisi de les dades 4 - Month }
...
```{r norm1 - de totes les variables cuantitatives }
...
fligner.test(Rainfall ~ WindSpeed9am, data = weatherMelb_complet)
